...
function EventPast() {
  ...
  const API_URL2 = `${process.env.NEXT_PUBLIC_APP_NEXTAUTH_URL}/event/past_events?search_month=${filteredMonth}&search=${debouncedSearchQuery}`;

  const { data, loading2, error2: error } = useAxiosFetch(
    API_URL2,
    {
    ...
    <div>
      <div className="flex flex-col lg:flex-row max-md:px-5 px-[25px]">
        ...
        <div className="lg:w-1/5 w-full">
          ...
          <div className="my-3 lg:my-10 text-left text-gray responsive-Text w-auto">
          ...
          {/* Previous and Next Month Buttons (Mobile) */}
          <div className="md:hidden flex justify-end gap-5 my-5 pr-3">
            <button onClick={handlePrevMonth} disabled={selectedMonth === 0}>
              ...
              >
                <g clipPath="url(#clip0_1410_132717)">
                  <path
        ...
        <div className="lg:w-4/5 w-full">
          <SwiperSlidePast data={data} loading={loading2} error={error} />
        </div>
      </div>
    </div>
  );
