...
function Quote() {
  ...
  useMemo(() => {
    if (topic) {
      setContent(topic?.result?.data?.content || "");
      setImages(topic?.result?.data?.attachments || []);
      setTitle(topic?.result?.data?.content || "");
    }
  }, [topic]);
  ...
  const handleGoBack = () => {
    router.push(`/members-only-content/Dashboard/member-forum/${quoteId}`);
  };
