...
function Quote() {
  ...
  const SaymanGoBack = () => {
    router.push(`/members-only-content/Dashboard/member-forum/${paramdId}`);
  };

  const paramdId = params.quoteId;

  const finalData = TOPICDETAILS.filter((data) => data.id == paramdId);
  const check = finalData.map((data) => data?.topic);
  const Content = check.map((data) => data.content);

  // Set the default content value
  const defaultContent = Content.length > 0 ? Content[0] : "";
