...
const MemberForum = () => {
  ...
  if (status === "loading" || !token) {
    return <Skeleton item={1} style={`h-[100px] w-full rounded-lg mb-3`} />;
  }

  if (error) {
    console.log(error);
    return <OopsSomeThingWrong />;
  }
        ...
        posts?.result.map((post, index) => (
          ...
          >
            <div className="flex items-start space-x-4">
              ...
              >
                <Image
                  //  src={session?.user?.userProfileUrl || "/dashboard/profile-pic.png"}
                  src={post?.attachments?.[0] || "/dashboard/profile-pic.png"}
                  alt="user-Image"
                  layout="fill"
              ...
              <div className="flex-1">
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-3 md:gap-5 my-1">
                    ...
                    >
                      {`${post?.user?.first_name || "N/A"} ${
                        post?.user?.last_name || "N/A"
                      }`}
                    </h2>
